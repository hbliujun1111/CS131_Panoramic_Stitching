...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        dx = [-1 0 1];
        img_dx = filter2(dx,currentImage);
        
        % gradients in y direction.
        img_dy = zeros(size(currentImage));
        dy = [-1;0;1];
        img_dy = filter2(dy,currentImage);
        ...
        for y = 1:4
            ...
            for x = 1:4
                %fprintf('cols£º%d \n',x);
                 ...
                 % Extract the patch from that window around the keypoint
                 sub_patch_mag = zeros(patch_size/4,patch_size/4);
                 sub_patch_theta = zeros(patch_size/4,patch_size/4);
                 sub_patch_mag = patch_mag(y_lo:y_hi,x_lo:x_hi);
                 sub_patch_theta = patch_theta(y_lo:y_hi,x_lo:x_hi);
               
                histogram = ComputeGradientHistogram(num_angles,sub_patch_mag,sub_patch_theta);
                feature = [feature histogram];
            end
        end
